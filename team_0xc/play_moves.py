...
class HelloWorld(cocos.layer.ColorLayer):
    ...
    def __init__(self):
        ...
        # being a CocosNode.
        person = {
            'left': cocos.sprite.Sprite('images/arm_sprite.png'),
            'right': cocos.sprite.Sprite('images/arm_sprite.png'),
            'head': cocos.sprite.Sprite('images/head_sprite.jpg')
        }

        print(person)

        # sprite in the center of the screen (default is 0,0)
        person['left'].position = 320, 240
        person['right'].position = 420, 240
        person['head'].position = 370, 260

        # sprite scale attribute starts with 3 (default 1 )
        person['left'].scale = 0.2
        person['right'].scale = 0.2
        person['head'].scale = 0.2

        # add the sprite as a child, but with z=1 (default is z=0).
        # this means that the sprite will be drawn on top of the label
        self.add(person['left'], z=1)
        self.add(person['right'], z=1)
