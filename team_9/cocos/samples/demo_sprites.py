...
class FreddyFirstMove(SpriteLayer):

    def on_enter(self):
        super(FreddyFirstMove, self).on_enter()

        sprite3 = Sprite(self.image)
        self.add(sprite3)
        sprite3.position = 0, 300

        mov = MoveBy((300, 0), 2)
        rot1 = RotateBy(10, 0.2)
        rot2 = RotateBy(-10, 0.2)

        mov2 = MoveBy((-150, 0), 2)

        mov3 = MoveBy((0, 30), 0.3)
        mov4 = MoveBy((0, -30), 0.3)

        sprite3.do(mov+rot1+rot2+rot1+rot2+rot1+rot2+rot1+rot2+
                   mov2+rot1+rot2+rot1+rot2+rot1+rot2+rot1+rot2
                   +Delay(0.5)+mov3+mov4+mov3+mov4+mov3+mov4+mov3+mov4)



class SpriteMoveBy(SpriteLayer):

    def on_enter(self, delta_x=0, delta_y=0):
...
tests = {
    ...
    4: ("FreddieMercury", "Under pressure live aid", FreddyFirstMove),
...
#    director.window.set_fullscreen(True)
    director.run(get_sprite_test(4))
